...
When you use ARR (Application Request Routing) as reverse proxy/ front end to your application server, requests to your application will first reach to the ARR server. And (as usual) if the client (i.e., Browser) to ARR communication is happening over HTTPS, after performing the SSL handshake on your application’s behalf, ARR forwards the client requests to the backend server which is running your application code. 
...
ARR detects the value of the `X-ARR-SSL` header, and it terminates the connection with http status code 400, when it finds any Non-HTTP.SYS/RFC complaint value in it.  As we have `O=HS` unknown character in the certificate properly.
